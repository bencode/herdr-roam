...
export const RuntimeSettings = ({
  ...
  const rows = [
    ...
    [
      'Version',
      snapshot.source.state === 'connected' ? snapshot.source.version : 'Unavailable',
    ],
    [
      'Protocol',
      snapshot.source.state === 'connected' ? String(snapshot.source.protocol) : 'Unavailable',
    ],
    ['Agents', snapshot.stale ? `${snapshot.items.length} cached in memory` : String(snapshot.items.length)],
  ] as const
        ...
        {rows.map(([label, value]) => (
          ...
          >
            ...
            <dd className="m-0 flex items-center gap-1.5">
              {label === 'Herdr' && <i className="size-1.75 rounded-full bg-primary" aria-hidden="true" />}
              {value}
            </dd>
          </div>
        ))}
